...
function wait_for_batch_jobs_to_finish {
  ...
  # $1 path to file with list of pbs job IDs
  # $1 ... $n pbs job IDs
  ...
  *)
    ...
    echo -n "... waiting for batch jobs to finish ... "
    local pjobids=$(sed 's/ /,/g'<<<$@)
  esac

  # do the waiting
  local qsubouterr=$(qsub --format="%i" -j$sjobids 2>&1)
  until [[ $qsubouterr = JOBID ]] || [[ $qsubouterr = "slurm_load_jobs error: Invalid job id specified" ]]
  do
    sleep 32
    qsubouterr=$(qsub --format="%i" -j$sjobids 2>&1)
  done  # qsubouterr
  echo done ...
...
function submit_wait_check_loop {
  # $1 ... $n qsub options (with leading hyphen)
  ...
  do
    ...
    local qsubouterr=$(qsub $@ 2>&1)
    while [[ $qsubouterr = *error* ]]
    do
      sleep 16
      qsubouterr=$(qsub $@ 2>&1)
    done
    echo $qsubouterr on $(date)
    local qjobid=$(echo $qsubouterr | cut -d ' ' -f 4)
    local qlogfile=qlogs/$(basename ${spath%sh})$sjobid

    # wait until slurm job is done
    wait_for_batch_jobs_to_finish $sjobid

    # check log file for error messages
    local nerrormessages=$(tr [:upper:] [:lower:] < $qlogfile | grep -c "$errormessages")
    case $nerrormessages in
    0)
      echo -n;;
    1)
      echo -n ... 1 error message found in log file $qlogfile ...;;
    *)
      echo -n ... $nerrormessages error messages found in log file $qlogfile ...;;
    esac  # nerrormessages
    iloop=$(($iloop + 1))
  done
  ...
  then
    echo
    echo tried $nloopmax times to qsub $@ without success !!! exiting ... $(date)
    exit 111
  else
    echo ... no error messages found in log file $qlogfile

    # return error code based on checksum
    local checksum=$(tail -n 1 $qlogfile | rev | cut -d ' ' -f 1 | rev)
...
function merge_reference_decades_if_necessary {
  ...
  ipaths=$(get_merge_reference_decades_ipaths $idirGCMdata/$2/${1}_${frequency}_${2}_ $3)
  echo ipaths are $ipaths
  exit_if_any_does_not_exist $ipaths
  local ysreference=$(cut -d '-' -f 1 <<<$3)
  local yereference=$(cut -d '-' -f 2 <<<$3)

  #IM NOT DOING THE MASKING STEP WS
  local lmerge=0; local ipath; for ipath in $ipaths; do [ $ipath -nt $4 ] && lmerge=1; done
  if [ $lmerge -eq 1 ]; then
    #merge the whole decades
    $cdo -mergetime $ipaths ${4}_tmp
    wait
    #then get the years we actually need:
    $cdo -selyear,$ysreference/$yereference ${4}_tmp $4
    wait
    #rm ${4}_tmp
  fi
  ...
  #  #local 
  #  cdopipe=$(get_merge_reference_decades_cdopipe $idirGCMdata/$2/${1}_${frequency}_${2}_ $3 $5)
  #  wait
  #  echo string to merge $(cdopipe)
  #  #$cdo -O mergetime $cdopipe $4
  #  #WS changed as can't do pipes with cdo on raijin for some reason
  #  $cdo -mergetime $cdopipe $4
  #  echo ... merging done
 #   echo
 # fi
  return 0

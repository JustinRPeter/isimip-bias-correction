...
for n=0L,(NUMLANDPOINTS-1) do begin
   ...
   print,'1st try: fitting an exponential distribution: y=B*x*(1-exp(-x/tau))'
   ...
   if (stat ne 0) then begin
      ...
      print,'generating new inital values:slope and offset, for non-linear fit'
      a = [alin(0),alin(1),.9]
      w = replicate(1.0,ndays)
      er = 1e33
      stat = 100
      ...
      if (stat ne 0) then begin
         print,'warning: 2nd initialisation failed'
         if (stat eq 1) then print,'The computation failed. Chi-square was increasing without bounds.'
         if (stat eq 2) then print,'The computation failed to converge in ITMAX iterations.'
   ...
   if (er gt 0.05) then begin
      print,'err is greater than 0.05, check if linear fit is better than non-linear fit'
      alin = a
      x0m = mean(x0)
      y0m = mean(y0)
      yx0m = mean(x0 * y0)
      ...
      if (er_lin le er) then begin
         print,'linear better than nonlinear'
